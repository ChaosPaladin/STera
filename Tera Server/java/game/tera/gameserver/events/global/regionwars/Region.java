...
public class Region extends SafeTask implements DieListener, PlayerSelectListener
{
	...
	public void onSelect(Player player)
	{
		...
		switch(getState())
		{
			...
			{
				player.sendPacket(CharSay.getInstance(Strings.EMPTY, "Битва за \"" + getName() + "\" начнется " + nextBattleDate, SayType.NOTICE_CHAT, 0, 0), true);
	...
	private synchronized void register(Npc npc, Player player)
	{
		...
		if(guildRank == null)
		{
			player.sendMessage("Вы не состоите в гильдии.");
			return;
		}
		...
		{
			player.sendMessage("Вы не являетесь мастером гильдии.");
			return;
		}
		...
		if(guild == getOwner())
		{
			player.sendMessage("Вы и так являетесь владельцом региона.");
			return;
		}
		...
		{
			player.sendMessage("Вы уже зарегестрированы на другой битве.");
			return;
		}
		...
		if(!dbManager.insertRegionGuildRegister(this, guild))
			player.sendMessage("Произошла ошибка, обратитесь в Тех Поддержку.");
		else
		{
			...
			if(event.isOwnerGuild(guild))
				player.sendMessage("Вы успешно зарегестрировались на битву, владение вашим регионом будет отменено при начале этой битвы.");
			else
				// сообщаем о успешной регистрации
				player.sendMessage("Вы успешно зарегестрировались на битву.");
		}
	}
	...
	private synchronized void unregister(Npc npc, Player player)
	{
		...
		if(!event.isRegisterGuild(guild))
		{
			player.sendMessage("Вы не зарегестрированы ни в какой битве.");
			return;
		}
		...
		// сообщаем об успехе
		player.sendMessage("Ваша гильдия была успешно отрегестрирована с битвы.");
	}
