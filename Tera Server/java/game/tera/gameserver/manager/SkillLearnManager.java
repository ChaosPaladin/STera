...
public final class SkillLearnManager
{
	...
	private SkillLearnManager()
	{
		...
		// создаем в ней списки
		for(int i = 0; i < 8; i++)
			learns[i] = Arrays.toArray(SkillLearn.class);

		// парсим изучаемые скилы
		...
		for(SkillLearn learn : result)
		{
			...
			// если в базе его нет или он не реализован, то пропускаем его
		...
		// перебираем все изучаемые скилы
	...
	{
		// получаем список всех изучаемых скилов для данного класса
		Array<SkillLearn> learnList = learns[player.getClassId()];

		// если их нет, сообщаем об этом и выходим
		if(learnList == null)
		{
		...
		// получаем таблицу текущих скилов игрока
		Table<IntKey, Skill> currentSkills = player.getSkills();

		// получаем массив изучаемых скилов
		...
		for(int i = 0, length = learnList.size(); i < length; i++)
		{
			...
			// если он уже изучен был, пропускаем
