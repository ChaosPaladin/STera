...
public final class CreateGuildDialog extends AbstractDialog
{
	...
	public synchronized boolean init()
	{
		...
		{
			// сообщаем и выходим
			player.sendMessage("У вас недостаточный уровень.");
			return false;
		}
		...
		{
			// сообщаем и выходим
			player.sendMessage("Вы уже состоите в гильдии.");
			return false;
		}
		...
		try
		{
			...
			for(int i = 0, length = members.size(); i < length; i++)
				...
				{
					/// сообщаем и выходим
					player.sendMessage("В вашей группе уже есть человек, состоящий в гильдии.");
					return false;
				}
		}
		finally
		...
		if(inventory.getMoney() < price)
		{
			...
			player.sendMessage("У вас недостаточно денег.");
			return false;
		}
